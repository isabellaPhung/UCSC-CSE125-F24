...
def test_each_runner(test_name, width_p, reset_val_p, sat_val_p):
...
@pytest.mark.parametrize("width_p,reset_val_p,sat_val_p", [(2, 1, 3), (7, 11, 67), (5, 30, 31), (4, 0, 11)])
@max_score(.5)
def test_all_runner(width_p, reset_val_p, sat_val_p):
    # This line must be first
    parameters = dict(locals())
...
@max_score(.5)
def test_lint(width_p, reset_val_p, sat_val_p):
    # This line must be first
    parameters = dict(locals())
...
@pytest.mark.parametrize("width_p, reset_val_p, sat_val_p", [(7, 11, 67)])
@max_score(.5)
def test_style(width_p, reset_val_p, sat_val_p):
    # This line must be first
    parameters = dict(locals())
