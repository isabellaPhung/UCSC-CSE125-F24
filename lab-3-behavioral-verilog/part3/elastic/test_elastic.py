...
def test_each_runner(test_name, width_p, datapath_reset_p, datapath_gate_p):
...
@pytest.mark.parametrize("datapath_gate_p", [0, 1])
@max_score(.5)
def test_all_runner(width_p, datapath_reset_p, datapath_gate_p):
    # This line must be first
    parameters = dict(locals())
...
@max_score(.125)
def test_lint(width_p, datapath_reset_p, datapath_gate_p):
    # This line must be first
    parameters = dict(locals())
...
@pytest.mark.skipif(not os.getenv('SIM').lower().startswith("verilator"), reason="Requires Verilator")
@max_score(.125)
def test_style(width_p, datapath_reset_p, datapath_gate_p):
    # This line must be first
    parameters = dict(locals())
