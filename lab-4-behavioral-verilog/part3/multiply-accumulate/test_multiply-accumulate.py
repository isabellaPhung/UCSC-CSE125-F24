...
@pytest.mark.parametrize("width_p", [18])
...
class MACModel():
    ...
    def produce(self):
        ...
        got = self._c_o.value
        a_i, b_i = self._q.get()
        if(self._cur_o + (a_i * b_i) > 2**48):
            self._cur_o = (self._cur_o + (a_i * b_i)) - 2**48
        else:
            self._cur_o = (self._cur_o + (a_i * b_i))
        expected = self._cur_o
        assert got == expected, f"Error! Result of {self._cur_o} + {a_i} * {b_i} does not match expected. Expected: {expected}. Got: {got}"
...
async def fuzz_edge_test(dut):
    """MAC a sequence of elements that expose edge cases at 50% line rate"""
    ...
    m = ModelRunner(dut, MACModel(dut))
...
async def fuzz_random_test(dut):
    """MAC a random sequence of data elements at 50% line rate"""
...
@cocotb.test()
async def fuzz_sequence_test(dut):
    """MAC a known sequence of data elements at 50% line rate"""

    # This is the InputModel
    seq = [*zip(range(-10,10, 1), range(10,-10, -1))]
    eg = SequenceGenerator(seq)
    l = len(seq)
    rate = .5
