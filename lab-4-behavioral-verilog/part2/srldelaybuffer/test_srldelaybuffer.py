...
@pytest.mark.parametrize("delay_p", [4, 2])
@pytest.mark.parametrize("test_name", tests)
def test_each_runner(test_name, width_p, delay_p):
...
@pytest.mark.parametrize("width_p", [7, 32])
@pytest.mark.parametrize("delay_p", [4, 1])
@max_score(1)
def test_all_runner(width_p, delay_p):
    # This line must be first
    parameters = dict(locals())
...
@max_score(0)
def test_lint(width_p, delay_p):
    # This line must be first
    parameters = dict(locals())
    lint(timescale, DIR_PATH, ["--lint-only"], parameters)

@pytest.mark.parametrize("width_p", [7])
@pytest.mark.parametrize("delay_p", [4])
@pytest.mark.skipif(not os.getenv('SIM').lower().startswith("verilator"), reason="Requires Verilator")
@max_score(0)
def test_style(width_p, delay_p):
    # This line must be first
    parameters = dict(locals())
...
class BufferModel():
    def __init__(self, dut):
        ...
        self._delay_p = dut.delay_p.value
        self._buf = [0] * self._delay_p
        self._deqs = 0
        self._enqs = 0
    ...
    def produce(self):
        ...
        expected = self._buf[0]
        self._buf[0] = self._q.get()
        self._buf = np.roll(self._buf, 1)
        if(self._deqs >= self._dut.delay_p.value):
            assert_resolvable(self._data_o)
            assert got == expected, f"Error! Output value on iteration {self._deqs} does not match expected. Expected: {expected}. Got: {got}"
        self._deqs += 1        
...
async def single_test(dut):
    ...
    m = ModelRunner(dut, BufferModel(dut))
...
async def out_fuzz_test(dut):
    """Transmit 4 * delay_p random data elements at 50% line rate (Output/Consumer is fuzzed)"""

    l = (1 << dut.delay_p.value) * 4
    rate = .5
...
async def in_fuzz_test(dut):
    """Transmit 4 * delay_p random data elements at 50% line rate (Input/Producer is fuzzed)"""
...
async def inout_fuzz_test(dut):
    """Transmit 4 * delay_p random data elements at ~25% line rate (Both are fuzzed)"""
...
async def full_bw_test(dut):
    """Transmit 8 * delay_p random data elements at 100% line rate"""

    # This is the InputModel
    l = (1 << dut.delay_p.value) * 8
    rate = 1
