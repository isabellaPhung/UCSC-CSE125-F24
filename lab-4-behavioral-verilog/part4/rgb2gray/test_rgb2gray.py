...
@pytest.mark.parametrize("width_p", [8])
...
class Rgb2grayModel():
    def __init__(self, dut):
        ...
        self._dut = dut
        self._red_i = dut.red_i
        self._green_i = dut.green_i
        self._blue_i = dut.blue_i
        self._gray_o = dut.gray_o

        # Model the latency-insensitive rgb2gray interface like a simple software queue
    ...
    def consume(self):
        assert_resolvable(self._red_i)
        assert_resolvable(self._green_i)
        assert_resolvable(self._blue_i)
        self._q.put((self._red_i.value.integer,
                     self._green_i.value.integer,
                     self._blue_i.value.integer))

    def produce(self):
        assert_resolvable(self._gray_o)
        got = self._gray_o.value.integer
        data_i = self._q.get()
        expected = int(np.floor(0.2989 * data_i[0] + 0.5870 * data_i[1] + 0.1140 * data_i[2]))
        
        self._gots.append(got)
        self._expecteds.append(expected)
        self._iter += 1
        rmse = self.rmse()
        print(f'On output iteration {self._iter}, expected: {expected}, got: {got}. Running RMSE: {rmse}')
        #assert got == expected, f"Error! RGB2GRAY output does not match expected. Expected: {expected}. Got: {got}"

    def rmse(self):
        return np.mean((np.array(self._gots) - np.array(self._expecteds))**2)**.5
...
class ImageDataGenerator():
    def __init__(self, dut, l):
        ...
        self._data = Image.open(os.path.join(DIR_PATH, "image.jpg")).getdata()
        print(self._data[0])
        self._loc = 0
...
class InputModel():
    ...
    async def _run(self):
        ...
        red_i = self._dut.red_i
        green_i = self._dut.green_i
        blue_i = self._dut.blue_i

        (red_i.value, green_i.value, blue_i.value) = (0,0,0)
        ...
        while self._nin < self._length:
            ...
            (red_i.value, green_i.value, blue_i.value) = data
...
@cocotb.test()
async def fuzz_constant_test(dut):
    """Stream constant data at 50% line rate"""

    # This is the InputModel
    eg = SingletonGenerator(dut, (16, 16, 16))
    l = 10
    rate = .5

    timeout = l * int(1/rate) * int(1/rate) * 2

    mm = Rgb2grayModel(dut)
    m = ModelRunner(dut, mm)
    ...
    rmse = mm.rmse()
    assert rmse <= 1, f"Error! Root Mean Squared Error (RMSE) greater than 1 at end of test"

@cocotb.test()
async def stream_constant_test(dut):
    """Stream constant data at 100% line rate"""
...
@cocotb.test()
async def fuzz_image_test(dut):
    """Stream "image" data at 50% line rate"""

    # This is the InputModel
    l = 100
    eg = ImageDataGenerator(dut, l)
    rate = .5
...
@cocotb.test()
async def stream_image_test(dut):
    """Stream image data at 100% line rate"""
...
@cocotb.test()
async def single_maxlat_test(dut):
    ...
    try:
        ...
        timeout = True
    assert timeout, "Error! Maximum latency expected for this module is two cycles."
...
@cocotb.test()
async def single_minlat_test(dut):
    """Test to transmit a single element in at most two cycles."""
    ...
    try:
        ...
        timeout = True
    assert timeout, "Error! Minimum latency expected for this module is two cycles."
